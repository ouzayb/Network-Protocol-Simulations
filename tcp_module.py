...
class tcp(object):
   ...
   class Sender(object):
      ...
      def recieve_ack(self, ack, command, callback_func):
         ...
         if(ack[0] == "HS-Ack"):
            if(ack[2] == False or ack[1] != self.tcp.count):
      ...
      def send_pkc_0(self, command, callback_func):
         ...
         command.write("sndpkt=make_pkt(HandShake,data,checksum)")
         ...
         callback_func(self.state, True, self.tcp.count, False, "HS")
         pass
      ...
      def send_pkc_1(self, command, callback_func):
         ...
         command.write("udp_send(data) from above")
         command.write("rdt_send(data)")
         command.write("sndpkt=make_pkt(Package,data,checksum)")
         command.write("udt_send(sndpkt)")
         command.write("start_timer")
         if(self.current_timer != None):
            print("The current timer wasn't null when trying to create a new timer. Stopping the old timer.")
         ...
         self.current_timer.start()
         # print(f"timer started {self.current_timer}")
         callback_func(self.state, True, self.tcp.count, False, "Pck")
         pass
      ...
      def send_pkc_0_again(self, command, callback_func):
         ...
         if(self.state == 1):
            ...
            self.current_timer.start()
         # print(f"timer started {self.current_timer}")
         callback_func(self.state, True, self.tcp.count, True, "HS")
         pass
      ...
      def send_pkc_1_again(self, command, callback_func):
         ...
         if(self.state == 3):
            ...
            self.current_timer.start()
         # print(f"timer started {self.current_timer}")
         callback_func(self.state, True, self.tcp.count, True, "Pck")
         pass
      
      @machine.output()
      def reset_to_wait1(self, command, callback_func):
         self.state = 2
         command.write("rdt_rcv(rcvpkt) && notcorrupt(rcvpkt) && isHS_ACK(rcvpkt)")
         command.write("stop_timer")
         command.write("Connection-opened-on-sender")
         self.current_timer.cancel()
         self.current_timer = None
      ...
      def reset_to_wait0(self, command, callback_func):
         self.state = 0
         command.write("rdt_rcv(rcvpkt) && notcorrupt(rcvpkt) && isPCK_ACK(rcvpkt)")
         command.write("stop_timer")
         command.write("Connection-closed-on-sender")
         self.current_timer.cancel()
         self.current_timer = None
         self.tcp.count = 1 - self.tcp.count
         # print(f"timer stopped {self.current_timer}")
         callback_func(self.state, False, 0)
         pass
      ...
      def reviece_wrong_ack0(self, command, callback_func):
         self.state = 1
         command.write("rdt_rcv(rcvpkt) && (corrupt(rcvpkt)|| isPCK_ACK(rcvpkt))")
         command.write("-Do Nothing-")
         callback_func(self.state, False, 0)
         pass
      ...
      def reviece_wrong_ack1(self, command, callback_func):
         self.state = 3
         command.write("rdt_rcv(rcvpkt) && (corrupt(rcvpkt)|| isHS_ACK(rcvpkt))")
         command.write("-Do Nothing-")
         callback_func(self.state, False, 0)
         pass
      ...
      wait_for_ack0.upon    (recieve_ack0           , enter = wait_for_ack1     , outputs = [reset_to_wait1, send_pkc_1]     )
   ...
   class Reciever(object):
      ...
      def recieve_pcg(self, message, command, callback_func):
         ...
         if message[0] == True:
            if(message[2] == "HS"):
               ...
               print(f"Reciever recieved a message that is not known. message type: {message[2]}")
         else:
      ...
      def send_ack0_first_time(self, command, callback_func):
         self.state = 1
         command.write("rdt_rcv(rcvpkt) && (notcorrupt(rcvpkt) || is_HandShake(rcvpkt))")
         command.write("extract(rcvpkt,data)")
         command.write("sndpkt=make_pkt(HS_ACK,checksum)")
         command.write("udt_send(sndpkt)")
         command.write("Connection-opened-on-reciever")
         callback_func(self.state, True, self.tcp.count, "HS-Ack")
         
      @machine.output()
      def send_ack1_first_time(self, command, callback_func):
         self.state = 0
         command.write("rdt_rcv(rcvpkt) && (notcorrupt(rcvpkt) || is_Package(rcvpkt))")
         command.write("extract(rcvpkt,data)")
         command.write("deliver_data(data)")
         command.write("sndpkt=make_pkt(PCK_ACK,checksum)")
         command.write("udt_send(sndpkt)")
         command.write("Connection-closed-on-reciever")
         callback_func(self.state, True, self.tcp.count, "Pck-Ack")
         
      @machine.output()
      def send_ack1_again(self, command, callback_func):
         self.state = 0
         command.write("rdt_rcv(rcvpkt) && (corrupt(rcvpkt) || is_Package(rcvpkt))")
         command.write("sndpkt=make_pkt(PCK_ACK,checksum)")
         command.write("udt_send(sndpkt)")
         callback_func(self.state, True, 1 - self.tcp.count, "HS-Ack")
         
      @machine.output()
      def send_ack0_again(self, command, callback_func):
         self.state = 1
         command.write("rdt_rcv(rcvpkt) && (corrupt(rcvpkt) || is_HandShake(rcvpkt))")
         command.write("sndpkt=make_pkt(HS_ACK,checksum)")
         command.write("udt_send(sndpkt)")
         callback_func(self.state, True, 1 - self.tcp.count, "Pck-Ack")
   ...
   def __init__(self):
      ...
      self.count = 0
      self.reciever = tcp.Reciever(self)
      self.sender = tcp.Sender(self)
